...
const UpdatePodcast = () => {
  ...
  const fetchPodcast = async () => {
    try {
      ...
      setFormData({
        ...
        fullVideoLink: podcast.fullVideoLink || '',
        section: podcast.section?._id || podcast.section || '',
        photo: podcast.thumbnail || '',
      });
      // Set preview if thumbnail exists
      if (podcast.thumbnail) {
        setPhotoPreview(podcast.thumbnail);
      }
    } catch (err) {
      setError(err.message);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If no new file but existing thumbnail, keep the existing thumbnail URL
      if (!photoFile && formData.photo) {
        podcastData.thumbnail = formData.photo;
      }

      await podcastAPI.update(id, podcastData, photoFile);
