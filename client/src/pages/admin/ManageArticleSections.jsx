...
import { articleSectionAPI } from '../../services/api';
import ErrorMessage from '../../components/common/ErrorMessage';
import LoadingSpinner from '../../components/common/LoadingSpinner';

const ManageArticleSections = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
      ...
      try {
        if (editingId) {
          await articleSectionAPI.update(editingId, formData);
        } else {
          await articleSectionAPI.create(formData);
        }
        setFormData({ name: '' });
        setShowForm(false);
        setEditingId(null);
  ...
  const handleDelete = async (id) => {
    try {
      await articleSectionAPI.delete(id);
    ...
    <>
      <div className="bg-white rounded-lg shadow-md p-8 max-w-4xl">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Manage Article Sections</h2>
          {!showForm && (
            <button
...
export default ManageArticleSections;
